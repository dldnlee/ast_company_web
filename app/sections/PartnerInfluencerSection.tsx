...
export function PartnerInfluencerSection() {
  return (
    <section className='flex flex-col items-center justify-center gap-10 py-16 px-10'>
      ...
      className='w-full'
      >
        {influencerNames.map((item, idx : number) => (
          <SwiperSlide key={idx} style={{ width: 'auto'}} className='flex justify-center items-center'>
            <div className=''>
              <img src={`/partner_influencers/${item.src}`} alt={item.src} className='rounded-2xl' />
              <p className='font-bold'>@{item.socialId}</p>
            </div>
          </SwiperSlide>  
        ))}
      </Swiper>
